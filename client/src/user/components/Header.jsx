...
function Header() {
  ...
  return (
    <header className="w-full flex justify-between items-center px-4 py-2 border-b shadow-md">
      {/* Logo */}
      <Link to={'/'} className="flex items-center gap-1">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-7">
          <path strokeLinecap="round" strokeLinejoin="round" d="m2.25 12 8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" />
        </svg>
        <span className="text-xl font-bold text-black">PhongTro365</span>
        <nav className="absolute left-1/2 transform -translate-x-1/2 flex space-x-20 text-gray-800">
          <Link to="/about" className="hover:text-blue-600">Về chúng tôi</Link>
          <Link to="/community" className="hover:text-primary">Cộng đồng</Link>
          <Link to="/contact" className="hover:text-purple-800">Liên hệ</Link>
        </nav>
      </Link>
      <div className='flex gap-2'>
        ...
        <div ref={menuRef} className="relative">
          ...
          {!!user && (
            <div>
              <div
                className="flex items-center gap-2 border border-gray-300 rounded-full py-1 px-1 cursor-pointer relative mr-40"
